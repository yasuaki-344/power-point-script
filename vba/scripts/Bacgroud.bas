Sub InsertMatrixVisual()
    ...
    Const shapeHeight As Single = (matrixHeight - spacing) / (1 + 1)
    Const startMatrixY As Single = startY + titleHeight + spacing
    Call AddBackgroundElements(slide, startX, startMatrixY, shapeWidth, shapeHeight, spacing, colCount)
End Sub
...
Private Sub AddBackgroundElements(slide As slide, startX As Single, startY As Single, shapeWidth As Single, shapeHeight As Single, spacing As Single, colCount As Integer)
    Dim col As Integer: For col = 0 To colCount - 1
        With slide.Shapes.AddShape(msoShapeRectangle, startX + (shapeWidth + spacing) * col, startY, shapeWidth, shapeHeight)
            .TextFrame.TextRange.Font.Size = 16
        End With

        With slide.Shapes.AddShape(msoShapeRectangle, startX + (shapeWidth + spacing) * col, startY + shapeHeight, shapeWidth, shapeHeight + spacing)
